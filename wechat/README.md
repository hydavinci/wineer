...
`0.4.2` 实际为146条酒款资料，六档分别为23、31、52、25、8、7条，距200款目标仍差54条；未用估价凑齐新增配额。自100条基线新增的46条均有具体容量、产品资料和独立记录的价格依据。当前全库容量覆盖108条、产品来源109条、版本标识46条、价格依据48条；仍有98条价格标为未核实估值。其余68条报价或身份待核实候选保存于 `data/catalog-candidates.json`，不打入运行时包，也不参与推荐。

此前将普通梦之蓝M9（52度500mL、非金M9）从候选移入900–1500元档，采用官方商城1099元活动展示价，1599元是划线原价；活动截止日、库存与结算未核实。商务送礼示例中，它与国窖1573等同分，因已有报价依据进入备选，不代表实饮评价优于其他酒。口子窖十年型改用匹配50度500mL单瓶来源，但仍无可采用的报价。

本轮新增泸州老窖1952（52度500mL单瓶），采用银行商城第三方供应商752元全额支付展示价，进入>500–900元档，不采用分期月供；购买资格、库存、运费与结算仍未核实。黄盖玻汾53度475mL改用苏宁聚合页56.50元单瓶展示价，两项均标为挂牌价，不是实际成交价。两条玻汾保留“疑似同款”提示，没有强行设置 `identityGroup`或共用报价。宝丰金标及限定年份红运郎仅补充候选证据，继续等待重复身份与版本核实；未采用报价及阻塞原因保存在台账。

商务送礼示例现为古井贡古20、泸州老窖1952、梦之蓝M9。古20得分123；1952、M9和国窖1573均为118且无偏好冲突，前两者有挂牌依据而国窖仍为估值；1952再因参考价低于M9排在其前。评分算法未改，变化来自入库资料，不代表实饮品质排序。
...
小程序采用暖白底（`#f7f6f2`）、墨黑文字及主按钮（`#242722`）、少量古铜金强调（`#80603a`）。内容区用白色、细边框和小圆角，不使用重阴影或渐变；导航栏和辅助海报使用同一套浅色配色，不引入外部字体、图片或 UI 库。

首页去掉整页外层卡片，问答使用两列选项；结果页突出调整偏好，分享、海报与重选保留为轻量操作。酒库使用紧凑列表，不重复显示“酒库资料”徽标。价格、规格、疑似同款及选购风险始终保留，不为了缩短卡片而截断风险说明。

主要按钮和选项保留至少44px高度；普通辅助文字及风险说明按至少4.5:1对比度设计。`tests/button-theme.test.js` 使用实际浏览器检查浅色主题、加载/禁用状态、320/390px选项布局和文字对比度；完成文档渲染后关闭独立测试浏览器，不等待可能滞留的 macOS Chrome 自然退出。这不替代微信真机验收。
